...
output_proportions_long["cumulative_n_operations"] = output_proportions_long["operation_id"].map(cumulative_n_operations_map)
...
pre = neuron_sequence.sequence_info.index.get_loc(3)
post = pre + 1

neuron_before = neuron_sequence.resolved_sequence[3]
neuron_after = neuron_sequence.resolved_sequence[5]

skeleton_before = neuron_before.to_skeleton_polydata()
skeleton_after = neuron_after.to_skeleton_polydata()

# plotter.subplot(0, 0)
plotter.add_mesh(skeleton_before, color="red", line_width=1.5, opacity=0.2)
# plotter.subplot(0, 1)
plotter.add_mesh(skeleton_after, color="blue", line_width=1.5, opacity=0.2)
